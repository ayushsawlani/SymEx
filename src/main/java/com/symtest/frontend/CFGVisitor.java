...
public class CFGVisitor extends CymbolBaseVisitor<Value> {
	...
	public Value visitAssign(CymbolParser.AssignContext ctx) {
		//System.out.println("*****************CFGVisitor visitAssign");
	...
	public Value visitReal(CymbolParser.IntContext ctx) {
		logger.finest("Real: " + ctx.getText());
		Value val = null;
		try {
			Integer realVal = Integer.parseInt(ctx.INT().getText());
			if (ctx.MINUS() != null)
				realVal = realVal * -1;
			ConcreteConstant constant = new ConcreteConstant(realVal, mCFG);
			val = new Value(constant);
		} catch (Exception e) {
			logger.severe("Real: " + e);
		}
		return val;
	}
	...
	public Value visitInput(CymbolParser.InputContext ctx) {
		//System.out.println("***************************CFGVisitor Input");
		logger.finest("Input");
		Value val = null;
		try {
	...
	public Value visitBooleanInput(CymbolParser.InputContext ctx) {
		//System.out.println("***************************CFGVisitor BoolInput");
		
		logger.finest("BooleanInput");
		Value val = null;
		try {
			BooleanInput inp = new BooleanInput(mCFG);
			val = new Value(inp);
		} catch (Exception e) {
			logger.severe("BooleanInput: " + e);
		}
		return val;
	}
	
	public Value visitRealInput(CymbolParser.InputContext ctx) {
		//System.out.println("***************************CFGVisitor RealInput");
		
		logger.finest("RealInput");
		Value val = null;
		try {
			RealInput inp = new RealInput(mCFG);
			val = new Value(inp);
		} catch (Exception e) {
			logger.severe("RealInput: " + e);
		}
		return val;
	}
}
